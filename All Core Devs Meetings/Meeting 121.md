...
uh here is the comment from Jacek that we should consider rest for this kind of api um I would I'm a bit unsure about the rest and I think that it will not suit as well for
...
some um message run trip the other thing is that rest is uh related like is about the resources which are some entities so I'm not sure if uh this uh
...
uh they are here now so there is the prepare payload which gives a comment to the execution client to stop building the um the payload uh it has these parameters um here and it will keep it up to date until the get payload is called and this and this process of producing the payload stops then
...
okay so so let's move on um yeah and execute payload so it verifies the payload um according to the execution enviornment rule set which is exposed in 

the EIP here is the question from Martin what if the parent block state is missing some error type will that be defined um this document like has 
...
oh I was going to say um and this is getting ahead of ourselves but I think in a sync protocol it is going to make sense for the beacon chain to 

be um optimistically processing forward without execution validation and I think that likely it's most simple to handle most of the most 
...
here but it depends on the sink uh entirely so um yeah uh the consensus validated message which is mapped on the proof state consensus 

validate if you end from the EIP um it's easy it's sent to the execution client by the consensus client when the when the beacon block gets 

validated with respect to the beacon chain um excecution or like the API says with respect to the consensus rule set so this is required 
...
options so the consensus validated may be sent like while the payload is being processed or after that um so this opens up uh like yeah the 
...
yeah yeah if it's invalid then you yeah get back so there are we can check this one now okay um yeah checking the chat okay cool engine fork 

choice updated um there is the PR to the to the EIP I'll drop it into the chat uh that unifies the two previous events which was the chain 

headset and the block finalized into the one so this document is uh matched the follows the EIP currently or vice versa anyway um here is the 
...
two thirds of the testers in the network uh they have been they have voted for it uh this is for JSON RPC for users JSON RPC um actually uh 
...
as well so um there is one out of this unification there is one note here this is more for consensus client developers um in the EIP this event 

should the finalized block hash before the transition before the first finalized block hash um it should be stopped with all zeros so this event 

will be we'll set we'll be sending the um actual hat block hash but the finalized block hash will will be stopped with all zeros before we get the 
...
merge fork we have we will have the execution payload in the block uh which filled with all zeroes so the finalized block hash um we will have 

this block hash already stopped serious I'm sorry I I could be a bit messy but you can read this and yeah it should be enough to um to 
...
oh just um the consensus validated means like I checked the proposer signature I checked the attestations and the other like kind of outer 
...
about that um and why ios super quiet now okay um yeah basically I was just saying that for the finalized block hash in particular the issue is 
...
writing to this to the uh um state as it is now um in exchange for making it harder to like go backwards and revert and revert to previous states 
...
point then the execution client can use that information to like basically like um dump all the information like jump to the journal and like flash memory and do all sorts of things that makes more efficient
...
here is the list the just proposed list of the new statuses for um the block for the json rpc a new identifier sends json rpc for the block so it 
...
okay cool um so block processing flow is here to illustrate yeah this couple of sequence diagrams it just illustrates how the er block will be 

processed I should probably add the focus stuff here the whole choice updated stuff here clarity I'll do that um now we are going through the 

transition process and which this is a very critical part um of this api um and yeah uh all the transition stuff and all the stuff that is 
...
in the next like updates um when the merge has already happened so we have here um like a couple of uh yeah we have here the couple of uh things 

that will help for the case when we would like to override the terminal total difficulty or set the terminal of work block which overrides the terminal total difficulty overwrites yeah so um these two methods yes sorry 
...
they are set from on their consensus client side um more on the reasoning behind this there is an issue here yep also by the way I forgot to 
...
I'm really sorry about this but for some reason it's not it doesn't always work um anyway um yeah but I while you dig it up I can also just um 
...
and kind of yeah just like basically tried to get the like good picture of it and then um peter basically went to on vacation and now basically 
...
likely take over and uh keep working on this so um this document that I just released yesterday is basically only really concerned with the sync 
...
the network obviously there it has to get to the real head of the chain and this in the end of the sync some um it will you know basically just 
...
they are generally less important in the context of this document but the proc is still used somewhere so this is why it exists but it's mostly 
...
uh I have one um regarding this um payloads execution after the sync is done uh when we got this message there are two options uh one is uh the 

execution client um stores all the execution payloads and then then when the sync is done it just executes them on top of the pivot block the 
...
right right the head with respect to attestations station beyond finality is also you know there's a degraded amount of security but it's uh yeah operating kind of in the same
...
basically um yeah so the the each one perspective is you know kind of you know like a mirror of what we just had so basically what happens is it 
...
validation step where it actually checks that the um um it also checks that the the downloaded headers match this checkpoint and this is just a 
...
actually below that so and then you go um basically through the block bodies and here you have two options you can either basically perform the 
...
to be done with something like the snap sync and um so the idea is that you will basically provide this and then what's really important to 

understand is also in the diagram um while um this while this like steps two and three are happening uh we are actually getting notifications 
...
care about it so we just say it's old and um or invalid and then if it's a future block then we restart the sync on this future block and the 
...
so basically um here we mostly talk about like how to handle the content of the database when there are multiple swing cycles and how to 

efficiently reuse um the information that was already stored in some previous sync uh we have a couple things here uh one is the handling of you 
...
reorg processing and i think this is actually going to be the main subject of the discussion in the upcoming week or two weeks basically um this 
...
handle and um we also described that basically like in order to facilitate the reorg processing it is recommended to then like keep other 

information in the main memory instead of the persistent storage because it just makes the reox a little bit easier and um finally we get into 

this part that should probably have way more text um so and it's kind of a bit of a controversial topic also no no it's not the issue section 

yet no for now we're still talking about the reorgs so we have the um we have this thing with the manual intervention reorg so basically um the 
...
problem that we discussed right away is that actually everything that I just said is you know like totally wrong because um finalization doesn't 
...
consensus finalization is something that can you know take up to two weeks in the worst case so what this means for us is that um our current 
...
too much so um we haven't we have basically been thinking about solutions last couple days how to really do it so um and what we find is that 
...
in the in the upcoming weeks and figure out uh how we can solve it in the in the best way um we will find a solution for this but yeah for now 
...
uh you mean there's the reverse divs um we don't really know about this so this is generally something that I that we need to discuss um so the problem with the reverse divs is that it's I'm actually not sure it it might be that aragon maybe someone from aragon asian can comment you know like how they handle the reorgs I think they might have something like this already implemented
...
it is manageable to like if you know aragon actually has it already implemented like this then we can definitely say that like this is a this is 
...
yeah thanks uh the other comment that i uh have is uh regarding this creative finality um so what could probably be used is the um like if the 
...
the most recent epoch uh the most recent uh the block at the most recent epoch boundary each time this boundary happens it could be used to um 
...
and decide what is the um like distance between them um and I think it makes sense for this calcified block conception and to use the blocks and 

the boundaries um at some follow distance uh from the head so this just basic thoughts on that yeah also we could use the justification stuff 
...
have huge implications because um it basically requires that during the certain reorgs need to be need to be handled you know like in in in some 
...
these cases and also um the the in general it's kind of like a bit messy because we're going to end up in a situation where like since the 
...
it would be good to have a recommendation that actually works and for now what I know is that not all clients are able to have such reorg um the 

it's it it puts some like for example in the case of aragon it's like yeah it's configurable but it will no longer be configurable in this in in 
...
that do not go to the depth of finality um I do note that in the event that you didn't have finality and in the event there's some sort of 
...
a place to attack um if you can get the chain to flip between states that are beyond the calcified state then you've now like grinded most 
...
well but if you want an available ledger then you have you don't have another choice um no I get it maybe one comment on that as though maybe 
...
only aragon has this exact system implemented right now so it was kind of you know like the way I wrote it was kind of inspired by how I think 
...
it's just a it's but again it's not really gonna be a guarantee because it it highly it's highly dependent on the actual um on the client 
...
pigeoned to the particular thing that we're dealing with um and I I have some ideas for that and I think that generally what you've written can be adapted to that uh but i just I think that's a good design goal
...
invaluable for aragon who I think generally relies on different sync uh protocols full sync and these rewinds
...
months we've seen like you know there can be external incentives like tokens of what not that that pop up to game this um especially as block space on
...
that the clients would behave in the same way regarding how they store the data and um like it means that uh it's break it's like in the future all the
...
aragon team is that we are against this this change but we are not going to die uh on this hill uh personally I think it's bad for two reasons first it if it requires a hard fork then it will distract us from the
