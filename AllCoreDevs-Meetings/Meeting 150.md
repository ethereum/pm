...
# **Summary:**
- Client's team agreed to fully shut down Ropsten by EOY. Broader testnet proposal to be discussed here: https://ethereum-magicians.org/t/proposal-predictable-ethereum-testnet-lifecycle/11575 
- Client teams went over the EIPs under discussion for a while now. No strong consensus on "what CFI means", "definition" is here (https://github.com/ethereum/execution-specs/tree/master/network-upgrades#definitions). With 4844 and EOF (all 5 EIPs) being on devnets already, they've been moved to CFI. For 4758, people wanted to see a better elaboration of the ways to deal with contract breakage before moving it to CFI. 2537 was also moved to CFI... but it's been CFI'd since Berlin. 
- The scope of Shanghai and when it should happen:  Hardest part to summarize given the diverging views, but there is strong (unanimous?) consensus that withdrawals should happen ASAP, and if we add anything alongside them in the fork, the delays to Shanghai should be minimal. Teams felt like a March fork with withdrawals should be possible. 
