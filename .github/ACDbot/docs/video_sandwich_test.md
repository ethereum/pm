...
- The required Zoom video layout is shared screen with active speaker. The workflow prefers Zoom's known-good `shared_screen_with_speaker_view(CC)` layout label, falls back to `shared_screen_with_speaker_view`, and logs which one was selected. It fails if neither combined layout is available so a Zoom recording-settings issue is visible.
- If Zoom provides a transcript, the workflow trims dead air from the main Zoom segment by keeping the range from the first transcript cue to the last transcript cue, with a small padding window on both sides.
- The workflow should not commit `ev.mp4` directly to the repository. The local file is about 89 MB, which is near GitHub's ordinary repository file limit and would bloat clone history. The first test asset is stored as the `ev.mp4` release asset on `acd-video-bumper-v1`; `ACD_BUMPER_URL` can override that URL later.
...
- `ACD_BUMPER_URL`: optional repository variable or secret pointing to a downloadable `ev.mp4`
- `ZOOM_CLIENT_ID`
- `ZOOM_CLIENT_SECRET`
- `ZOOM_REFRESH_TOKEN`
