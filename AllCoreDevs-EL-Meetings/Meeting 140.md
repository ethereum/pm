...
0:31:48	Mikhail Kalinin:	gossip of terminal blocks is important for transition, for the case when we have more than one terminal block and proposer may switch from 
one to another -- we must be sure that all of them are disseminated
...
0:40:42	Adrian Sutton:	Yeah we could request the block later and that may be something we should recommend somewhere.  Currently the CL has no way of knowing that 
the EL needs more time to prepare the block.
...
0:56:46	protolambda:	If there will be V2, there is a compatible extension to the payload attributes that would also be really useful for L2 and testing. Simply a 
"transactions" list payload attribute to always process in the block (drop any invalid transactions).
...
1:00:26	Ansgar Dietrichs:	I think the gas limit is less of a governance and more of an oracle mechanism. the job of the entities in control should only "inform the 
network about the current gas limit desired by the protocol”, nothing more. so I am not convinced validators are the best entities to perform this oracle service
...
1:01:12	Mikhail Kalinin:	Right, but it’s not defined how CL will use this versions. Like will CL switch to V2 only if? How would CL know that V2 is supported by 
EL?
...
1:03:31	Mikhail Kalinin:	What if we have two new methods in the future that are related to each other and CL behaviour depends on whether they are supported or 
not. It would probably be valuable to request the version of ENgine API that is supported by EL and then make a decision, though and Unexpected method error may 
also work
...
1:10:19	Adrian Sutton:	Part of that confusion is that the CL is syncing from genesis which takes time just because there aren't good sources for checkpoint sync 
states currently.
...
1:13:24	Micah Zoltu:	So Peter is now speaking through someone else's mic and he still sounds like he is in an echoing background.... I'm starting to think that is 
just the sound of Peter's voice.
...
1:25:06	danny:	I do think it reasonable to quantify if/how a test case can happen and if engineering burden is undue wrt likelihood and damage to network as a 
whole, to deprioritize for the next few months

1:25:06	Łukasz Rozmej:	in getpayload?

1:27:59	Thomas Jay Rush:	We should save time for a discussion of difficulty bomb.

1:28:09	Mikhail Kalinin:	fwiw, TBH on EL side = TTD + whitelisting a chain given a block hash. The former is a TTD machinery as we know it, the latter I guess is 
also supported by all EL clients
...
1:42:11	Ansgar Dietrichs:	I think the main issue here is what “when the bomb hits" means - 0.6s delay in November for me means that with 5133 the bomb would really 
only "hit" in November or after, which would be too much of a delay for my taste - then we can also just completely remove it
...
1:43:13	Tomasz Stańczak:	maybe you are not affected by 14 seconds blocks but mining pools see rewards falling and it is seen by them and others, also basefee will 
probably be going up causing more cost to the users

1:43:14	Ansgar Dietrichs:	(late August, maybe early September)

1:43:50	Łukasz Rozmej:	On related topic of forking next testnet from our side it would be optimal to do next testnet fork in ~5 weeks, but ~4 weeks is also 
acceptable
...
1:47:25	Mikhail Kalinin:	IMO, we should fork Sepolia as soon as client devs fix encountered during merge on Ropsten. And after that fork Goerly with the same 
client versions that are going to make it into the Mainnet fork with having enough time before the Mainnet fork to assess the results we will get from Goerly
...
1:48:20	Adrian Sutton:	Yep got to keep rolling on merging testnets as that's a long pole for getting mainnet to merge. And merging testnets keeps pushing the 
community/users to be ready which is a really key part.

1:48:26	Bordel:	acd on skip week<<1

1:48:27	Bordel:	woo

1:49:06	Ansgar Dietrichs:	I think a decision like “we will delay & have the defusal fork at block height X, but we will pick an exact number of blocks offline over 
the next few days” would be ideal

1:49:19	Micah Zoltu:	I would support that.

1:49:49	Adrian Sutton:	Also +1 to pushing the bomb back on the basis that we have to choose a TTD to time the merge which is much harder while the bomb is going 
off.
...
1:52:06	Ansgar Dietrichs:	the numbers don't seem to be consistent, if we will only have 0.6s block delays in November with a 2.5 month delay, that would mean we 
would only get to 0.6s delay in mid August without

1:52:29	Tomasz Stańczak:	let us please confirm implementation of the EIP today with details to be filled in

1:52:58	danny:	have to run. if bomb is delayed, it should start going off in August. putting it in september jeapordizes its effect on preventing a pow fork. 
additioanlly, I agree with Ben -- we need to keep moving
...
1:54:07	Thomas Jay Rush:	At 14 second blocks, 100,000 blocks takes about two weeks. So if it's set back 500,000 blocks, we would be about the same place as we are 
today in 2.5 months (Mid-August). It doesn't matter when it's set back. The same will be true. That's not how it works.
